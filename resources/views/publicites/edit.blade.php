@extends('layout.app')
@section('title')
Modifier une publicité
@stop

@section('content')

    <h1>Modifier une publicité</h1>
    <hr/>

    {!! Form::model($pubs, [
        'method' => 'POST',
        'url' => ['manager/publicites', $pubs->id],
        'class' => 'form-horizontal',
        'files'=>true
    ]) !!}

    @if ($errors->any())
        <ul class="alert alert-danger">
            @foreach ($errors->all() as $error)
                <li>{{ $error }}</li>
            @endforeach
        </ul>
    @endif
    <?php $position = [0=>"Slide principale", 1=>"Annexe slide 1", 2=>"Annexe slide 2",3=>"Menu de gauche", 4=>"Pages infos generales", 5=>"Mega menu"]  ?>

            <div class="form-group {{ $errors->has('nom') ? 'has-error' : ''}}">
                <div class="row">
                    {!! Form::label('libelle', 'Libelle(Texte 2): ', ['class' => 'col-sm-3 control-label']) !!}
                    <div class="col-sm-8">
                        {!! Form::text('libelle', $pubs->libelle, ['class' => 'form-control']) !!}
                        {!! $errors->first('libelle', '<p class="help-block">:message</p>') !!}
                    </div>
                </div>
            </div>
            <div class="form-group {{ $errors->has('nom') ? 'has-error' : ''}}">
                <div class="row">
                    {!! Form::label('text1', 'Texte 1: ', ['class' => 'col-sm-3 control-label']) !!}
                    <div class="col-sm-8">
                        {!! Form::text('text1', $pubs->text1, ['class' => 'form-control']) !!}
                        {!! $errors->first('text1', '<p class="help-block">:message</p>') !!}
                    </div>
                </div>
            </div>
            <div class="form-group {{ $errors->has('nom') ? 'has-error' : ''}}">
                <div class="row">
                    {!! Form::label('text3', 'Texte 3: ', ['class' => 'col-sm-3 control-label']) !!}
                    <div class="col-sm-8">
                        {!! Form::text('text3',$pubs->text3, ['class' => 'form-control']) !!}
                        {!! $errors->first('text3', '<p class="help-block">:message</p>') !!}
                    </div>
                </div>
            </div>
            <div class="form-group {{ $errors->has('nom') ? 'has-error' : ''}}">
                <div class="row">
                    {!! Form::label('text4', 'Texte 4: ', ['class' => 'col-sm-3 control-label']) !!}
                    <div class="col-sm-8">
                        {!! Form::text('text4',$pubs->text4, ['class' => 'form-control']) !!}
                        {!! $errors->first('text4', '<p class="help-block">:message</p>') !!}
                    </div>
                </div>
            </div>

            <div class="form-group {{ $errors->has('date_debut') ? 'has-error' : ''}}">
                <div class="row">
                    {!! Form::label('date_debut', 'Date Debut: ', ['class' => 'col-sm-3 control-label']) !!}
                    <div class="col-sm-8">
                        {!! Form::date('date_debut', date_format(date_create($pubs->date_debut), "Y-m-d"), ['class' => 'form-control']) !!}
                        {!! $errors->first('date_debut', '<p class="help-block">:message</p>') !!}
                    </div>
                </div>
            </div>
            


            <div class="form-group {{ $errors->has('date_fin') ? 'has-error' : ''}}">
                <div class="row">
                    {!! Form::label('date_fin', 'Date Fin: ', ['class' => 'col-sm-3 control-label']) !!}
                    <div class="col-sm-8">
                        {!! Form::date('date_fin', date_format(date_create($pubs->date_fin), "Y-m-d"), ['class' => 'form-control']) !!}
                        {!! $errors->first('date_fin', '<p class="help-block">:message</p>') !!}
                    </div>
                </div>
            </div>

            <div class="form-group {{ $errors->has('image') ? 'has-error' : ''}}">
                <div class="row">
                    {!! Form::label('statut', 'Afficher la pub: ', ['class' => 'col-sm-3 control-label']) !!}
                    <div class="col-sm-9">
                        {!! Form::checkbox('statut', ($pubs->statut==1), ['class' => 'form-control', 'name'=>"statut"]) !!}
                        {!! $errors->first('statut', '<p class="help-block">:message</p>') !!}
                    </div>
                </div>
            </div>


            <div class="form-group {{ $errors->has('position') ? 'has-error' : ''}}">
                <div class="row">
                    {!! Form::label('position', 'Position: ', ['class' => 'col-sm-3 control-label']) !!}
                    <div class="col-sm-8">
                        {!! Form::select('position', \App\PubsPositions::pluck('libelle','id'), $pubs->position, ['placeholder' => 'Choisissez une position','class' => 'form-control','id'=>'position_select']) !!}
                        {!! $errors->first('position', '<p class="help-block">:message</p>') !!}
                    </div>
                </div>    
            </div>

            <div class="form-group {{ $errors->has('link') ? 'has-error' : ''}}">
                <div class="row">
                    {!! Form::label('link', 'Lien de details: ', ['class' => 'col-sm-3 control-label']) !!}
                    <div class="col-sm-8">
                        {!! Form::text('link',$pubs->link, ['class' => 'form-control']) !!}
                        {!! $errors->first('link', '<p class="help-block">:message</p>') !!}
                    </div>
                </div>
            </div>

            <div class="col-md-12">
                     <div class="file-upload" id='image'>
                        <div class="image-upload-wrap">
                            <input class="file-upload-input" type='file' onchange="readURL(this);"
                                accept="image/*" name="image"  id="input-image" tar="image" value="{{asset('storage/uploads/pubs/'.$pubs->image)}}" />
                            <div class="drag-text">
                                <h3 id="imglabel">Image</h3>
                            </div>
                        </div>
                        <div class="file-upload-content " style="{{$pubs->image != null ? 'display:block' : ''}}">
                            <img class="file-upload-image" style="max-width:100%;max-height:100%" src="{{asset('storage/uploads/pubs/'.$pubs->image)}}" alt="your image"/>
                            <div class="image-title-wrap">
                                <div class="row">
                                    <button type="button" onclick="removeUpload('image')" class="remove-image btn-sm">Changer
                                        <span class="image-title"> Image chargée </span> </button>
                                </div>
                            </div>
                        </div>
                       </div>
               </div>
            

    <div class="form-group">
        <div class="col-sm-offset-3 col-sm-3">
            {!! Form::submit('ENREGISTRER', ['class' => 'btn btn-primary form-control']) !!}
        </div>
    </div>
    {!! Form::close() !!}



@endsection


@section('js')
<script>
$(document).ready(function(){
    $( "#position_select" ).change(function(e) {
        $.ajax({
         url: "/publicites/dimensionsposition",
         data: { positionid: this.value },
         type: "GET",
         beforeSend: function(xhr){xhr.setRequestHeader('Accept', 'application/json');},
         success: function(data) { 
             $('#imglabel').html(data.x+' X '+data.y);
             removeUpload('image');

          }
      });
    });

    

});
</script>
@endsection