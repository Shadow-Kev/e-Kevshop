...
@section('content')
    ...
    <h1>Modes de paiement <a href="{{ url('modepaiement/create') }}" class="btn btn-primary pull-right btn-sm">+ Mode de paiement</a></h1>
    <hr>
    <div class="table table-responsive">
            ...
            @foreach($modepaiement as $item)
                <tr>
                    <td>{{ $item->id }}</td>
                    <td><a href="{{ url('modepaiement', $item->id) }}">{{ $item->nom }}</a></td>
                    <td>{{ $item->description }}</td>
                    <td>{{ $item->commande }}</td>
                    <td>
                        <a href="{{ url('modepaiement/' . $item->id . '/edit') }}" class="btn btn-primary btn-xs"> <span><i class="fa fa-edit"></i></span></a>
